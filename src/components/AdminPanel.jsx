...
function AdminPanel() {
    ...
    async function cargarDatos() {
        try {
            ...
            const [dataProd, dataCat, dataUser] = await Promise.all(
    ...
    const handleEliminarProducto = async (id) => {
        if (window.confirm('¿Seguro que quieres eliminar este producto?')) {
            try {
                ...
                setAlerta({ variant: 'success', message: 'Producto eliminado con éxito.' });
                cargarDatos();
            } catch (error) { setAlerta({ variant: 'danger', message: `FALLO AL ELIMINAR: ${error.message}` }); }
        }
    };
    ...
    // ----------------------------------------------------
    // --- 5. FUNCIONES CRUD DE CATEGORÍAS ---
    // ----------------------------------------------------

    const handleEliminarCategoria = async (id) => {
        setAlertaCategoria(null);
        if (window.confirm('¿Seguro que quieres eliminar esta categoría? Si hay productos asociados, fallará.')) {
            try {
                const response = await fetch(`http://localhost:8080/api/categorias/${id}`, { method: 'DELETE' });
                if (!response.ok) throw new Error(`Error ${response.status}`);
                
                await cargarDatos();
                setAlertaCategoria({ variant: 'success', message: 'Categoría eliminada con éxito.' });
            } catch (error) { setAlertaCategoria({ variant: 'danger', message: `FALLO AL ELIMINAR: ${error.message}` }); }
        }
    };
        ...
        <Container fluid className="p-0">
            <Row className="g-0" style={{ minHeight: '100vh' }}>
                ...
                <Col md={2} className="bg-dark text-white p-4 d-none d-md-block">
                    <h4 className="fw-bold text-warning mb-4">KJM ADMIN</h4>
                    <ul className="list-unstyled">
                        <li className="mb-3"><a href="#" onClick={() => setKey('productos')} className={`text-decoration-none d-flex align-items-center ${key === 'productos' ? 'text-warning fw-bold' : 'text-white'}`}><BoxSeam className="me-2"/> Productos</a></li>
                        <li className="mb-3"><a href="#" onClick={() => setKey('categorias')} className={`text-decoration-none d-flex align-items-center ${key === 'categorias' ? 'text-warning fw-bold' : 'text-white'}`}><Tags className="me-2"/> Categorías</a></li>
                        <li className="mb-3"><a href="#" onClick={() => setKey('usuarios')} className={`text-decoration-none d-flex align-items-center ${key === 'usuarios' ? 'text-warning fw-bold' : 'text-white'}`}><People className="me-2"/> Usuarios</a></li>
                        <li className="mb-3"><a href="#" onClick={() => setKey('reportes')} className={`text-decoration-none d-flex align-items-center ${key === 'reportes' ? 'text-warning fw-bold' : 'text-white'}`}><FileBarGraph className="me-2"/> Reportes</a></li>
                    </ul>
                </Col>
                ...
                <Col md={10} className="bg-light p-5">
                    ...
                    {alerta && (<Alert variant={alerta.variant} onClose={() => setAlerta(null)} dismissible className="mb-4">{alerta.message}</Alert>)}
                    ...
                    <Tabs id="admin-tabs" activeKey={key} onSelect={(k) => setKey(k)} className="mb-3">
                        ...
                        <Tab eventKey="productos" title="Productos">
                            <Card className="shadow-sm border-0">
                                <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center"><h5 className="mb-0 fw-bold">Lista de Productos ({productos.length})</h5><Button variant="primary" onClick={() => abrirModalProducto()}><Plus size={20} /> Nuevo Producto</Button></Card.Header>
                        ...
                        <Tab eventKey="categorias" title="Categorías">
                            <Card className="shadow-sm border-0">
                                ...
                                <Card.Body>
                                    {alertaCategoria && (<Alert variant={alertaCategoria.variant} onClose={() => setAlertaCategoria(null)} dismissible>{alertaCategoria.message}</Alert>)}
                                    <Table hover responsive className="mb-0 align-middle">
                                        <thead className="bg-light"><tr><th>ID</th><th>Nombre</th><th className="text-center">Acciones</th></tr></thead>
                                        <tbody>
                                            {categorias.map((cat) => (<tr key={cat.id}><td>{cat.id}</td><td><Badge bg="info" className="text-dark text-uppercase">{cat.nombre}</Badge></td><td className="text-center"><Button variant="warning" size="sm" className="me-2 text-white" onClick={() => abrirModalCategoria(cat)}><Pencil /></Button><Button variant="danger" size="sm" onClick={() => handleEliminarCategoria(cat.id)}><Trash /></Button></td></tr>))}
                                        </tbody>
                                    </Table>
                                </Card.Body>
                            </Card>
                        ...
                        <Tab eventKey="usuarios" title="Usuarios">
                            <Card className="shadow-sm border-0">
                                ...
                                <Card.Body>
                                    {alertaUsuario && (<Alert variant={alertaUsuario.variant} onClose={() => setAlertaUsuario(null)} dismissible>{alertaUsuario.message}</Alert>)}
                                    <Table hover responsive className="mb-0 align-middle">
                                        <thead className="bg-light">
